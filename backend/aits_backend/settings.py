...
AITS_PACK_PROFILE = os.getenv('AITS_PACK_PROFILE', '').strip()
LLM_EVAL_API_PACK = AITS_PACK_PROFILE == 'llm-eval-api'

if LLM_EVAL_API_PACK:
    _PACK_DISABLED_APPS = frozenset({'web_testing', 'app_testing', 'performance'})
    INSTALLED_APPS = [app for app in INSTALLED_APPS if app not in _PACK_DISABLED_APPS]
...
# Sonic 云测（后端代理 Controller API；未配置 SONIC_BASE_URL 时无法拉取套件/调设备）
SONIC_BASE_URL = os.getenv('SONIC_BASE_URL', '').strip().rstrip('/')
SONIC_API_PREFIX = os.getenv('SONIC_API_PREFIX', '/server/api').strip().rstrip('/')
SONIC_TIMEOUT_SECONDS = int(os.getenv('SONIC_TIMEOUT_SECONDS', '15') or '15')
SONIC_USERNAME = os.getenv('SONIC_USERNAME', '').strip()
SONIC_PASSWORD = os.getenv('SONIC_PASSWORD', '').strip()
SONIC_TOKEN = os.getenv('SONIC_TOKEN', '').strip()
SONIC_PROJECT_ID = int(os.getenv('SONIC_PROJECT_ID', '0') or 0)
...
PERFORMANCE_REPORTS_ROOT = os.path.join(MEDIA_ROOT, 'data', 'reports', 'performance')
PROMETHEUS_SD_AUTO_UPDATE = os.getenv('PROMETHEUS_SD_AUTO_UPDATE', 'True').lower() == 'true'
PROMETHEUS_SD_TARGET_FILE = os.path.join(BASE_DIR, 'aits_monitor', 'prometheus', 'targets.json')
...
# Grafana AI 告警默认接收对象（notification_receiver）
# 优先按 ID 精确匹配；未配置时可按名称关键字匹配（兜底再使用代码内默认）
GRAFANA_AI_NOTIFY_RECEIVER_ID = os.getenv('GRAFANA_AI_NOTIFY_RECEIVER_ID', '').strip()
GRAFANA_AI_NOTIFY_RECEIVER_NAME_KEYWORD = os.getenv(
    'GRAFANA_AI_NOTIFY_RECEIVER_NAME_KEYWORD', 'AI测试部门群'
).strip()

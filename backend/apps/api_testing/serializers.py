...
class APISpecificationCreateSerializer(serializers.ModelSerializer):
    ...
    额外接收两个仅在 Postman 导入时生效的可选字段：
    - import_mode: spec_only / endpoint_cases / scenario_case / all
    - enable_ai_translate: 是否调用 LLM 翻译 pm.test / prerequest 脚本
    它们不入 APISpecification 主表字段，而是由 View 写入 metadata。
    """

    import_mode = serializers.ChoiceField(
        choices=['spec_only', 'endpoint_cases', 'scenario_case', 'all'],
        required=False,
        default='spec_only',
        write_only=True,
        help_text='Postman 导入模式',
    )
    enable_ai_translate = serializers.BooleanField(
        required=False,
        default=True,
        write_only=True,
        help_text='Postman 导入时是否启用 AI 翻译 pm.test 脚本',
    )

    class Meta:
        model = APISpecification
        fields = ['spec_type', 'spec_name', 'description', 'import_mode', 'enable_ai_translate']

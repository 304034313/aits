...
# ============ Postman Collection 导入任务 ============

# 路径变量识别（在去除 `?` 之前的 path 上提取 {id} 占位符）
_PATH_VAR_RE = re.compile(r'\{([A-Za-z_][A-Za-z0-9_]*)\}')

# region agent log
def _dbg_postman_log(location: str, message: str, data: Any,
                     hypothesis_id: str = '', run_id: str = 'postman-debug') -> None:
    """Debug 模式专用：写一行 NDJSON 到工作区根目录 debug 日志。"""
    try:
        import time as _t
        import os as _os
        ...
        with open(log_path, 'a', encoding='utf-8') as _f:
            _f.write(json.dumps(payload, ensure_ascii=False, default=str) + '\n')
    except Exception:
        pass
# endregion


@shared_task(bind=True, name='api_testing.tasks.import_postman_collection_async')
def import_postman_collection_async(self, spec_id: int, import_mode: str = 'spec_only',
                                    enable_ai_translate: bool = True,
                                    user_id: Optional[int] = None) -> Dict[str, Any]:
    """异步导入 Postman Collection。

    Args:
        spec_id: APISpecification ID（spec_type=postman，由 view 层预先创建）
        import_mode: 导入模式
            - spec_only: 仅生成 API 规范 + 端点
            - endpoint_cases: 端点 + 每个请求一个端点测试用例
            - scenario_case: 端点 + 一个整 collection 的场景测试用例
            - all: 同时生成端点用例 + 场景用例
        enable_ai_translate: 是否启用 LLM 翻译 pm.test/prerequest 脚本
        user_id: 触发用户 ID
    ...
    return execute_async_task_with_websocket(
        self,
        'Postman导入',
        _execute_postman_import,
        spec_id, import_mode, enable_ai_translate, user_id,
    )


def _execute_postman_import(task_instance, spec_id: int, import_mode: str,
                            enable_ai_translate: bool,
                            user_id: Optional[int]) -> Dict[str, Any]:
    """实际执行 Postman 导入。"""
    from api_testing.postman_parser_service import (
        PostmanParserService,
        create_endpoints_from_postman,
    )

    User = get_user_model()
    spec: Optional[APISpecification] = None

    try:
        update_task_progress(task_instance, 5, '正在加载 API 规范记录...')
        spec = APISpecification.objects.select_related('uploaded_file', 'project').get(id=spec_id)

        # 标记 spec 为处理中
        spec.status = APISpecification.TaskStatus.RUNNING
        spec.error_message = ''
        spec.save(update_fields=['status', 'error_message'])

        # region agent log
        _dbg_postman_log(
            'tasks.py:_execute_postman_import:entry',
            'Postman 导入任务入口',
            {'spec_id': spec_id, 'import_mode': import_mode,
             'enable_ai_translate': enable_ai_translate,
             'user_id': user_id,
             'spec_metadata_at_start': spec.metadata},
            hypothesis_id='A',
        )
        # endregion

        if not spec.uploaded_file:
            return _postman_import_failed(spec, 'API 规范未关联文件，无法导入')
        ...
        update_task_progress(task_instance, 10, '正在读取并校验 Postman Collection...')
        parser = PostmanParserService()
        try:
            collection = parser.load_file(spec.uploaded_file)
            parse_result = parser.parse(collection)
        except ValueError as ve:
            return _postman_import_failed(spec, str(ve))
        except Exception as e:
            logger.error(f"Postman 解析异常: {e}", exc_info=True)
            return _postman_import_failed(spec, f'Postman 解析失败: {str(e)}')

        # 写回 metadata（保留之前由 view 写入的 import_mode / enable_ai_translate）
        existing_meta = spec.metadata or {}
        merged_meta = {**existing_meta, **(parse_result.get('metadata') or {})}
        spec.metadata = merged_meta
        spec.parsed_content = ''  # 不再使用 parsed_content，避免重复占用 DB
        spec.save(update_fields=['metadata', 'parsed_content'])

        update_task_progress(task_instance, 20, '正在创建 API 端点...')
        endpoints = create_endpoints_from_postman(spec, parse_result)

        # region agent log
        _dbg_postman_log(
            'tasks.py:_execute_postman_import:after_parse',
            'Postman 解析完成',
            {'endpoints_count': len(endpoints),
             'parse_metadata': parse_result.get('metadata'),
             'first_endpoint_raw': (
                 getattr(endpoints[0], '_postman_raw', None) if endpoints else None
             )},
            hypothesis_id='B,D,F',
        )
        # endregion
        ...
        if not endpoints:
            return _postman_import_failed(spec, '未从 Postman Collection 中解析到任何请求')

        created_cases: List[APITestCase] = []
        scenario_case: Optional[APITestCase] = None
        translation_warnings: List[str] = []

        base_url = merged_meta.get('base_url') or ''
        global_variables = merged_meta.get('variables') or {}

        if import_mode in ('endpoint_cases', 'all'):
            update_task_progress(task_instance, 30, '正在为每个请求生成端点测试用例...')
            created_cases, translation_warnings = _generate_endpoint_cases(
        ...
        if import_mode in ('scenario_case', 'all'):
            update_task_progress(task_instance, 82, '正在生成场景测试用例...')
            try:
                scenario_case = _generate_scenario_case(
                    spec=spec,
                    endpoints=endpoints,
                    user=user,
                    enable_ai_translate=enable_ai_translate,
                ...
                logger.error(f"生成场景测试用例失败: {e}", exc_info=True)
                translation_warnings.append(f'场景用例生成失败: {e}')

        update_task_progress(task_instance, 95, '正在更新 API 规范状态...')
        spec.status = APISpecification.TaskStatus.COMPLETED
        spec.error_message = ''
        spec.save(update_fields=['status', 'error_message'])
        ...
        return {
            'success': True,
            'status': 'completed',
            'message': 'Postman Collection 导入完成',
            'spec_id': spec.id,
            'endpoints_count': len(endpoints),
            'endpoint_cases_count': len(created_cases),
            'scenario_case_id': scenario_case.id if scenario_case else None,
        ...
        return build_error_result(None, f'API 规范不存在: id={spec_id}')
    except Exception as e:
        error_msg = f'Postman 导入异常: {str(e)}'
        logger.error(error_msg, exc_info=True)
        if spec is not None:
            try:
                spec.status = APISpecification.TaskStatus.FAILED
                spec.error_message = error_msg
                spec.save(update_fields=['status', 'error_message'])
            except Exception:
                pass
        return build_error_result(None, error_msg)
...
def _postman_import_failed(spec: APISpecification, error_message: str) -> Dict[str, Any]:
    """统一处理失败：更新 spec 状态并返回 build_error_result。"""
    try:
        spec.status = APISpecification.TaskStatus.FAILED
        spec.error_message = error_message
        spec.save(update_fields=['status', 'error_message'])
    except Exception as e:
        logger.warning(f"更新 spec 失败状态时异常: {e}")
    return build_error_result(None, error_message)


def _generate_endpoint_cases(task_instance, spec: APISpecification,
                             endpoints: List[Any], user, enable_ai_translate: bool,
                             base_url: str, global_variables: Dict[str, Any],
                             progress_start: int, progress_end: int) -> Tuple[List[APITestCase], List[str]]:
    """为每个 Postman 请求生成一个端点测试用例。

    返回：
        (created_cases, warning_messages)
    """
    from ai_core.postman_script_translator import (
        get_postman_script_translator,
        PostmanScriptTranslator,
    )

    translator: Optional[PostmanScriptTranslator] = None
    if enable_ai_translate:
        translator = get_postman_script_translator()
    ...
    for idx, endpoint in enumerate(endpoints):
        progress = progress_start + int(span * (idx / max(total, 1)))
        update_task_progress(
            task_instance,
            progress,
            f'正在生成端点用例 ({idx + 1}/{total}): {endpoint.method} {endpoint.path}',
        )

        raw = getattr(endpoint, '_postman_raw', {}) or {}
        events = raw.get('events') or {}
        test_script = events.get('test', '')
        prerequest_script = events.get('prerequest', '')

        # region agent log
        _dbg_postman_log(
            'tasks.py:_generate_endpoint_cases:per_endpoint',
            f'端点 {idx + 1}/{total} 原始数据快照',
            {'method': endpoint.method, 'path': endpoint.path,
             'raw_present': bool(raw),
             'has_body': bool(raw.get('body')),
             'body_content_types': list((raw.get('body') or {}).get('content', {}).keys()),
             'headers_count': len(raw.get('headers') or []),
             'query_count': len(raw.get('query') or []),
             'raw_query': raw.get('query') or [],
             'raw_headers': raw.get('headers') or [],
             'test_script_len': len(test_script),
             'prerequest_script_len': len(prerequest_script),
             'test_script_preview': test_script[:500],
             'prerequest_script_preview': prerequest_script[:500]},
            hypothesis_id='D,F',
            run_id='postman-debug-post-fix',
        )
        # endregion

        translated = _translate_or_default(
        ...
        # region agent log
        _dbg_postman_log(
            'tasks.py:_generate_endpoint_cases:translated',
            '翻译器返回结果',
            {'method': endpoint.method, 'path': endpoint.path,
             'validators': translated.get('validators'),
             'extract': translated.get('extract'),
             'variables': translated.get('variables'),
             'unrecognized_len': len(translated.get('unrecognized_raw') or ''),
        ...
        try:
            step = _build_endpoint_teststep(endpoint, raw, translated, global_variables)
            # region agent log
            _dbg_postman_log(
                'tasks.py:_generate_endpoint_cases:final_step',
                '最终 HttpRunner teststep',
                {'method': endpoint.method, 'path': endpoint.path,
                 'step_request_keys': list(step.get('request', {}).keys()),
                 'step_request_method': step.get('request', {}).get('method'),
                 'step_request_url': step.get('request', {}).get('url'),
                 'step_request_json_type': type(step.get('request', {}).get('json')).__name__,
                 'step_request_json_preview': (
                     json.dumps(step.get('request', {}).get('json'), ensure_ascii=False)[:500]
                     if step.get('request', {}).get('json') is not None else None
                 ),
                 'step_request_data_preview': (
                     json.dumps(step.get('request', {}).get('data'), ensure_ascii=False, default=str)[:500]
                     if step.get('request', {}).get('data') is not None else None
                 ),
                 'step_request_headers': step.get('request', {}).get('headers'),
                 'step_validate': step.get('validate'),
            ...
            # endregion
            hrun_json = {
                'config': _build_endpoint_config(endpoint, base_url, global_variables),
                'teststeps': [step],
            }
            script_content = json.dumps(hrun_json, ensure_ascii=False, indent=2)

            description = _compose_case_description(
                endpoint.description or '',
                translated.get('unrecognized_raw', ''),
                translated.get('debugtalk_snippets') or [],
            )
            ...
            with transaction.atomic():
                test_case = APITestCase.objects.create(
                    ...
                    title=raw.get('name') or f"{endpoint.method} {endpoint.path}",
            ...
            logger.error(f"创建 Postman 端点用例失败: {endpoint.method} {endpoint.path}, err={e}", exc_info=True)
            warnings.append(f'{endpoint.method} {endpoint.path}: {e}')

    return created_cases, warnings


def _generate_scenario_case(spec: APISpecification, endpoints: List[Any], user,
                            enable_ai_translate: bool, base_url: str,
                            global_variables: Dict[str, Any]) -> APITestCase:
    """把整个 Postman Collection 组装为一个场景测试用例。"""
    from ai_core.postman_script_translator import (
        get_postman_script_translator,
        PostmanScriptTranslator,
    )
    ...
    for endpoint in endpoints:
        ...
        unrecognized = translated.get('unrecognized_raw', '')
        if unrecognized:
            unrecognized_parts.append(f'### {endpoint.method} {endpoint.path}\n{unrecognized}')

    hrun_json = {
        'config': {
            'name': f"Postman 场景: {spec.spec_name or spec.file_name}",
    ...
    description_parts.append(f'由 Postman Collection 自动导入，共 {len(teststeps)} 个步骤')
    if unrecognized_parts:
        description_parts.append('### 待人工迁移的脚本片段\n' + '\n\n'.join(unrecognized_parts))

    with transaction.atomic():
        return APITestCase.objects.create(
            test_case_type='scenario',
            project=spec.project,
            title=f"Postman 场景: {spec.spec_name or spec.file_name}",
...
def _translate_or_default(translator, enable_ai_translate: bool,
                          test_script: str, prerequest_script: str,
                          endpoint) -> Dict[str, Any]:
    """统一调用翻译器，无法可用时返回默认 200 断言结构。"""
    if not enable_ai_translate or translator is None:
        # 不启用 AI：直接走规则回退（仍会识别最常见模式）
        from ai_core.postman_script_translator import PostmanScriptTranslator
        return PostmanScriptTranslator().fallback_translation(test_script, prerequest_script)
    ...
    try:
        ...
        logger.warning(f"翻译失败，回退到默认 validator: {e}")
        return {
            'validators': [{'eq': ['status_code', 200]}],
            'extract': {},
            'variables': {},
            'debugtalk_snippets': [],
            'unrecognized_raw': '\n'.join(filter(None, [test_script, prerequest_script])),
            'notes': f'翻译失败回退: {e}',
        }


def _build_endpoint_config(endpoint, base_url: str,
                           global_variables: Dict[str, Any]) -> Dict[str, Any]:
    """构造单条端点用例的 HttpRunner config。"""
    return {
...
def _build_endpoint_teststep(endpoint, raw: Dict[str, Any],
                             translated: Dict[str, Any],
                             global_variables: Dict[str, Any]) -> Dict[str, Any]:
    """根据 Postman 原始信息 + AI 翻译结果，组装单个 HttpRunner teststep。"""
    method = endpoint.method
    path = endpoint.path  # 已经是 {id} 风格

    # 将路径变量映射为 HttpRunner 变量（{id} -> $id），并补全默认变量值
    path_vars = _PATH_VAR_RE.findall(path)
    step_variables: Dict[str, Any] = {}
    url_path = path
    for var in path_vars:
        url_path = url_path.replace('{' + var + '}', '$' + var)
        if var in (translated.get('variables') or {}):
            step_variables[var] = translated['variables'][var]
        elif var in global_variables:
            step_variables[var] = global_variables[var]
        else:
            step_variables[var] = ''  # 占位，待用户在执行前补充

    # 合并 AI 识别出的其他变量赋值（非路径变量）
    for k, v in (translated.get('variables') or {}).items():
        if k not in step_variables:
            step_variables[k] = v

    # 构造 query/headers/body
    request: Dict[str, Any] = {
        'method': method,
        'url': url_path,
    }

    params = _query_list_to_dict(raw.get('query') or [])
    if params:
        request['params'] = params

    headers = _header_list_to_dict(raw.get('headers') or [])
    if headers:
    ...
    if isinstance(body_content, dict) and body_content:
        # 优先使用 application/json，其次表单/原文
        if 'application/json' in body_content:
            example = body_content['application/json'].get('example')
            if example not in (None, ''):
                request['json'] = example
        elif 'application/x-www-form-urlencoded' in body_content:
            example = body_content['application/x-www-form-urlencoded'].get('example') or {}
            request['data'] = example
            request.setdefault('headers', {})['Content-Type'] = 'application/x-www-form-urlencoded'
        elif 'multipart/form-data' in body_content:
            example = body_content['multipart/form-data'].get('example') or {}
            request['data'] = example
            request.setdefault('headers', {})['Content-Type'] = 'multipart/form-data'
        elif 'text/plain' in body_content:
            example = body_content['text/plain'].get('example') or ''
            request['data'] = example
        else:
            # 其他类型：取第一个
            first_ct, first_payload = next(iter(body_content.items()))
            example = first_payload.get('example')
            if example not in (None, ''):
                request['data'] = example
    ...
    extract = translated.get('extract') or {}
    if extract:
        step['extract'] = extract

    validators = translated.get('validators') or [{'eq': ['status_code', 200]}]
    step['validate'] = _clean_validators(validators)

    return step


def _query_list_to_dict(query: List[Dict[str, Any]]) -> Dict[str, Any]:
    result: Dict[str, Any] = {}
    for q in query:
        key = q.get('key')
        if not key:
            continue
        result[key] = q.get('value', '')
    return result


def _header_list_to_dict(headers: List[Dict[str, Any]]) -> Dict[str, Any]:
    """转 dict 时跳过常见自动头，避免 HttpRunner 重复构造。"""
    skip_keys = {'content-length', 'host', 'user-agent', 'accept-encoding', 'connection'}
    result: Dict[str, Any] = {}
    for h in headers:
        key = h.get('key', '')
...
def _clean_validators(validators: List[Any]) -> List[Dict[str, Any]]:
    """统一 validator 为 {comparator: [field, expected]} 字典格式。"""
    ...
    for v in validators or []:
        if isinstance(v, dict):
            ...
            comparator = v[0]
            params = v[1] if isinstance(v[1], list) else [v[1]]
            cleaned.append({comparator: params})
    if not cleaned:
        cleaned.append({'eq': ['status_code', 200]})
    return cleaned


def _compose_case_description(original: str, unrecognized: str,
                              debugtalk_snippets: List[str]) -> str:
    """组装测试用例的 description 字段。"""
    parts: List[str] = []
    if original:
        parts.append(original)
    if unrecognized:
        parts.append('### 待人工迁移的 Postman 脚本\n' + unrecognized)
    if debugtalk_snippets:
        parts.append(
            '### AI 建议添加到 debugtalk.py 的函数\n```python\n'
            + '\n\n'.join(debugtalk_snippets)
            + '\n```'
        )
    return '\n\n'.join(parts)

...
import json
import logging
from typing import Dict, List, Any, Optional, TypedDict, Callable, Union, Set, Tuple
...
class TestStep(BaseModel):
    ...
    assertions: List[Dict[str, Any]] = Field(
        ...
        description="该步骤完成后的结构化断言列表（DSL，详见 assertion_schema.AssertionSpec）",
    )
...
class TestCase(BaseModel):
    ...
    expected_result: str = Field(description="预期结果（自由文本，兼容字段；建议同步用 expectations 给出结构化断言）")
    expectations: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="用例级结构化断言列表（DSL，与 expected_result 并存）",
    )
...
class WebUITestCaseGeneratorState(TypedDict):
    ...
    module_id: Optional[int]           # 用户选择的模块ID（module 模式用，用于提取UI资产）
    related_module_ids: Optional[List[int]]  # scenario 模式下用户 @ 的相关模块集合
    test_case_type: str                # 'module' | 'scenario'，本次生成的用例类型
    user_id: int                       # 用户ID
    ...
    created_test_cases: Optional[List[Dict[str, Any]]]  # 创建的测试用例列表（仅 auto_save=True 时填充）
    candidate_test_cases: Optional[List[Dict[str, Any]]]  # 候选用例列表（auto_save=False 时填充，供前端人工审核）
    test_cases_count: Optional[int]    # 创建的测试用例数量
    current_step: str                  # 当前执行步骤
    execution_timeline: List[Dict[str, Any]]  # 执行时间线
    error_message: Optional[str]       # 错误信息
    rag_context_missing: Optional[bool]  # RAG检索是否未命中当前项目的需求文档
    scenario_quality_warnings: Optional[List[str]]  # 场景质量门剔除/警示信息列表
    auto_save: Optional[bool]          # True=AI直接入库；False=只产出候选，由用户人工审核后批量入库（默认 False）
...
class WebUITestCaseGenerator:
    ...
    def run(
        ...
        test_case_type: str = "module",
        related_module_ids: Optional[List[int]] = None,
        ...
        Args:
            ...
            module_id: 用户选择的模块ID（module 模式用，用于提取UI资产）
            test_case_type: 'module' | 'scenario'，本次生成的用例类型
            related_module_ids: scenario 模式下用户 @ 的相关模块集合
            auto_save: True 则 AI 生成完直接入库；False（默认）则只产出候选列表，
                由前端展示给用户人工挑选后再调用批量创建接口入库
        ...
        try:
            logger.info(
                f"开始运行Web UI测试用例生成器，类型: {test_case_type}，"
                f"自动入库: {auto_save}，用户输入: {user_input}"
            )
    ...
    def _run_with_langgraph(
        ...
        """使用LangGraph工作流运行"""
        # 归一化用例类型，避免脏值
        normalized_type = test_case_type if test_case_type in ("module", "scenario") else "module"
        normalized_related_ids: List[int] = []
        for raw_id in (related_module_ids or []):
            try:
                normalized_related_ids.append(int(raw_id))
        ...
        initial_state = WebUITestCaseGeneratorState(
            ...
            module_id=module_id,
            related_module_ids=normalized_related_ids,
            test_case_type=normalized_type,
            user_id=self.user_id,
            test_case_name=None,
            ...
            execution_timeline=[],
            error_message=None,
            rag_context_missing=None,
            scenario_quality_warnings=None,
        ...
        rag_context_missing = bool(result.get("rag_context_missing"))
        candidate_test_cases: List[Dict[str, Any]] = result.get("candidate_test_cases") or []

        # 根据是否自动入库选择基础文案
        if auto_save:
            base_message = f'Web UI测试用例生成成功，共创建 {test_cases_count} 个测试用例'
        else:
            candidate_count = len(candidate_test_cases)
            base_message = (
                f'AI 已生成 {candidate_count} 条候选用例，请在对话中勾选保留后入库'
                if candidate_count > 0
                else 'AI 生成完成，但本次没有产出候选用例，请调整描述后重试'
            )

        warning_message = (
            '⚠ 本次未检索到当前项目的需求文档作为上下文，生成的用例可能与实际业务存在偏差，建议人工复核或先上传需求文档后重新生成。'
        )

        # 合并所有 warnings：场景质量门剔除信息 + RAG 缺失提示
        scenario_warnings: List[str] = list(result.get("scenario_quality_warnings") or [])
        all_warnings: List[str] = list(scenario_warnings)
        if rag_context_missing:
            all_warnings.append(warning_message)

        # 组装最终消息：基础消息 + 质量门简报 + RAG 提示
        message_parts: List[str] = [base_message]
        if scenario_warnings:
            flagged_count = sum(1 for w in scenario_warnings if '标记为不达标' in w or '已丢弃不达标用例' in w)
            if flagged_count:
                message_parts.append(
                    f'其中 {flagged_count} 条已标记为不达标（候选中标红，请人工决定是否保留）'
                    if not auto_save else f'已丢弃 {flagged_count} 条不达标用例'
                )
        if rag_context_missing:
        ...
        return {
            ...
            "user_input": user_input,
            "project_id": project_id,
            "test_case_type": result.get("test_case_type") or normalized_type,
            ...
            "candidate_test_cases": candidate_test_cases,
            "test_cases_count": test_cases_count,
            "test_case_id": result.get("test_case_id"),  # 保持向后兼容
            "rag_context_missing": rag_context_missing,
    ...
    def _requirement_parsing_node(self, state: WebUITestCaseGeneratorState) -> WebUITestCaseGeneratorState:
        ...
        try:
            ...
            # 标记本次是否拿到了项目需求文档作为业务上下文，供最终结果汇总加 warning
            state["rag_context_missing"] = len(business_context) == 0
            if state["rag_context_missing"]:
                logger.warning(
                    f"RAG未检索到 project_id={project_id} 的需求文档上下文，"
                    f"用例生成将仅依赖模块/页面/元素结构信息，质量可能受影响"
                )

            # 按用例类型加载 UI 资产：
            # - module 模式：仅当前 module_id 对应的页面+元素
            # - scenario 模式：related_module_ids 涉及的所有模块的页面+元素，按模块分组渲染
            test_case_type = state.get("test_case_type") or "module"
            ui_assets_context = ""
            try:
                from web_testing.models import WebPage

                if test_case_type == "scenario":
                    related_ids = state.get("related_module_ids") or []
                    if related_ids:
                        # 把所选模块的页面拉一遍，按模块分组渲染，便于 LLM 跨模块组装步骤
                        modules_qs = (
                            WebUITestModule.objects
                            .filter(id__in=related_ids)
                            .order_by('order', 'id')
                            .values_list('id', 'name')
                        )
                        module_id_to_name = {mid: name for mid, name in modules_qs}

                        pages = (
                            WebPage.objects
                            .filter(module_id__in=related_ids)
                        ...
                        if pages.exists():
                            ui_assets_context = "【跨模块场景涉及的页面与元素资产库（按模块分组）】\n"
                            last_module_id = None
                            for page in pages:
                                if page.module_id != last_module_id:
                                    mod_name = module_id_to_name.get(page.module_id, '未命名模块')
                                    ui_assets_context += f"\n# 模块: {mod_name} (id={page.module_id})\n"
                                    last_module_id = page.module_id
                ...
                logger.warning(f"获取UI资产失败 (type={test_case_type}): {e}")
    ...
    def _test_designing_node(self, state: WebUITestCaseGeneratorState) -> WebUITestCaseGeneratorState:
        ...
        try:
            ...
            auth_profiles_context = "（项目尚未配置 Auth Profile）"
            if project_id:
                try:
                ...
                try:
                    from web_testing.models import WebAuthProfile
                    profiles = WebAuthProfile.objects.filter(
                        environment__project_id=project_id
                    ).values_list('name', flat=True)
                    if profiles:
                        names = ", ".join(profiles)
                        auth_profiles_context = (
                            f"项目已配置 Auth Profile：{names}。"
                            "对于已登录后的业务场景用例：preconditions 写「依赖 Auth Profile: <名称>」，"
                            "steps 中不要重复生成 UI 登录步骤（输入账号/密码、点击登录按钮等）。"
                        )
                except Exception as e:
                    logger.warning(f"查询 Auth Profile 失败: {e}")

            # 按类型差异化的 prompt 提示词
            if test_case_type == "scenario":
                type_hint = (
                    "【本次任务类型：跨模块场景用例 - 强约束】\n"
                    "本次生成端到端业务流用例。必须严格遵守以下硬性要求（违反者将被自动丢弃）：\n"
                    "  - 禁止生成只在单一模块内执行的用例\n"
                    "  - 禁止把『输入边界变种』（密码超长、用户名特殊字符、空值等单点测试）作为场景用例\n"
                    "  - 每条用例的 steps 必须触达 ≥ 2 个不同模块的页面/元素\n"
                    "  - 每条用例的 steps 长度必须 ≥ 5 步（端到端业务流通常需要 5-10 步）\n"
                    "  - 每一条用例的 `module_id` 字段统一固定填 null（不归属任何单一模块）\n"
                    "在【UI 资产库】中你会看到按模块分组的页面和元素，请充分利用这些跨模块资产组装端到端步骤。"
                )
                coverage_rule = (
                    "3. 【场景覆盖维度 - 流程视角】（场景用例专用，不再使用『输入边界』维度）：\n"
                    "   - 主成功路径（happy path）：完整走通端到端业务流\n"
                    "   - 关键失败分支：流程中关键步骤失败时的处理（如下单时库存不足、提交时鉴权过期）\n"
                    "   - 中断与恢复：用户中断后再回来能否恢复（如未登录被跳转登录后、登录成功能回到原页面）\n"
                    "   - 数据流转校验：跨模块的数据传递一致性（如订单金额 = 购物车金额）"
                )
                count_rule = (
                    "4. 【精简有效】只生成 3 到 5 条最具代表性的端到端流程用例，宁缺勿滥；"
                    "每条都必须是有业务价值的关键路径，禁止凑数。"
                )
            else:
                type_hint = (
                    "【本次任务类型：模块用例】\n"
                    "本次生成的是【模块用例】，所有步骤应聚焦在用户指定的单个业务模块内部。\n"
                    "请根据用例的语义，从下方业务模块列表中选择最匹配的 ID 填入 `module_id`；"
                    "如果不匹配任何模块，请填 null。"
                )
                coverage_rule = (
            ...
            # 场景模式：附一个 good example 作为格式范本（提升 LLM 对端到端流程的理解）
            good_example_block = ""
            if test_case_type == "scenario":
                good_example_block = (
                    "\n【场景用例标准范例】（仅作格式与跨度参考，业务请按用户输入设计）：\n"
                    "一个合格的跨模块场景用例形态如下 —— 8 步、跨 2 个模块（登录 + 订单）：\n"
                    "```json\n"
                    "{\n"
                    '  "id": "TC001",\n'
                    '  "title": "用户登录后下单查看订单详情",\n'
                    '  "module_id": null,\n'
                    '  "description": "验证从登录到下单到查看订单全流程的端到端业务正确性",\n'
                    '  "priority": "high",\n'
                    '  "category": "functional",\n'
                    '  "preconditions": ["用户已注册账号", "商品已上架"],\n'
                    '  "steps": [\n'
                    '    {"step_id":1, "action":"goto", "target":"", "value":"/login", "description":"打开登录页"},\n'
                    '    {"step_id":2, "action":"input", "target":"用户名输入框", "value":"valid_user", "description":"输入用户名（登录模块）"},\n'
                    '    {"step_id":3, "action":"input", "target":"密码输入框", "value":"valid_pwd", "description":"输入密码（登录模块）"},\n'
                    '    {"step_id":4, "action":"click", "target":"登录按钮", "value":"", "description":"提交登录（登录模块）"},\n'
                    '    {"step_id":5, "action":"click", "target":"商品卡片", "value":"", "description":"选择商品（订单模块）"},\n'
                    '    {"step_id":6, "action":"click", "target":"立即下单按钮", "value":"", "description":"点击下单（订单模块）"},\n'
                    '    {"step_id":7, "action":"click", "target":"确认订单按钮", "value":"", "description":"确认订单（订单模块）"},\n'
                    '    {"step_id":8, "action":"assert_text", "target":"订单状态", "value":"待支付", "description":"校验订单状态（订单模块）"}\n'
                    '  ],\n'
                    '  "expected_result": "登录成功 → 下单成功 → 订单详情页能看到该订单且状态为『待支付』"\n'
                    "}\n"
                    "```\n"
                    "注意：每个步骤的 description 末尾标注它属于哪个模块，便于自检『是否真的跨模块』。"
                )

            # 动态提取标准动作资产及参数约束规则
            # 优先使用 description（含语义/适用边界，例如 select vs custom_select 的区分），其次回退 label
            allowed_actions_text = ""
            for action_key, config in WEBUI_STEPS_MAPPING.items():
                desc = config.get("description") or config.get("label", action_key)
                ...
                allowed_actions_text += f"- `{action_key}` —— {desc}\n    参数: target {target_rule}, value {value_rule}\n"
            ...
            【登录态 Auth Profile 说明】
            {auth_profiles_context}
            preconditions 仅作人类可读说明，不驱动 Playwright 执行；机器可执行的登录态由用例字段 auth_profile_id 绑定。
            ...
            输出格式要求（每个用例必须同时给出 expected_result 与 expectations 两个字段；每个 step 可附 assertions 列表）：
            {{
            "test_cases": [
                {{
                "id": "TC001",
            ...
            【断言 DSL 规范】（expectations 数组中每条断言遵循以下结构）：
            - 字段：type / locator / expected / timeout_ms / soft / description
            - type 必须为下列之一：
              text_visible（页面文本可见）、text_equals（元素文本等于）、text_contains（元素文本包含）、
              text_not_exists（页面不存在文本）、url_equals、url_matches（URL 匹配正则）、
              element_visible、element_hidden、element_enabled、element_disabled、
              attribute_equals（expected 形如 {{"name":"value","expected":"xxx"}}）、
              count_equals（expected 为整数）、count_at_least（expected 为整数下限）、
              network_response（expected 形如 {{"url_pattern":".*/api/x.*","status":200}}），
              vision_assert（expected 为自然语言；仅在 DOM 难以表达时使用，比如"页面右上角显示用户头像"）。
            - locator 形如 {{"by": "text|css|role|test_id|xpath|page_object", "value": "..."}}；
              页面级断言（url_*、network_response、vision_assert）允许 locator 省略。
            - soft 默认 true（强烈推荐 true，仅在该条断言失败必须中断时设 false）。
            - timeout_ms 默认 5000，范围 500~30000。
            - description 用一句中文描述断言意图，将显示在 Allure 报告里。
            - 数量要求：每个用例建议给出 1~3 条 expectations；至多 1 条 vision_assert，避免昂贵。

            示例（expectations 字段）：
            [
              {{"type":"url_matches","expected":"/dashboard","timeout_ms":5000,"soft":true,"description":"登录成功后应跳转 dashboard"}},
              {{"type":"text_visible","locator":{{"by":"text","value":"欢迎"}},"expected":"欢迎","soft":true,"description":"页面应出现欢迎语"}}
            ]
            示例（steps[i].assertions 字段）：
            [
              {{"type":"element_visible","locator":{{"by":"css","value":".order-list .row"}},"soft":true,"description":"订单列表行应出现"}}
            ]
            ...
            messages = prompt.format_messages(
                ...
                good_example_block=good_example_block,
                modules_context=modules_context,
                auth_profiles_context=auth_profiles_context,
    ...
    def _scenario_quality_gate(
        self,
        test_cases: List[Dict[str, Any]],
        related_module_ids: List[int]
    ) -> Tuple[List[Dict[str, Any]], List[Dict[str, str]]]:
        """场景用例质量门：对不达标的用例打标，不再剔除

        判定红线：
          - 步骤数 len(steps) >= 5
          - 涉及模块数 >= 2（通过 step.target 反查 WebElement 推断）

        Args:
            test_cases: LLM 生成的原始用例列表
            related_module_ids: 用户在前端 @ 选中的相关模块 ID 列表

        Returns:
            (flagged_cases, discarded_records)
            - flagged_cases: 与入参等长的用例列表，对每条追加 `_quality_flag = {'ok': bool, 'reason': str}`，
              达标 ok=True、reason=''；不达标 ok=False、reason 为具体原因。
              这里不再丢弃任何用例，最终是否入库交由前端用户人工挑选。
            - discarded_records: 与不达标用例一一对应的清单（仅供 warnings 文案使用），
              元素结构: {'title': str, 'reason': str}
        """
        MIN_STEPS = 5
        MIN_MODULES = 2

        # 构建 element.name -> set(module_id) 的反查映射（同名元素可能存在于多个模块）
        name_to_modules: Dict[str, Set[int]] = {}
        if related_module_ids:
            try:
                from web_testing.models import WebElement
                for el in WebElement.objects.filter(
                    page__module_id__in=related_module_ids
                ).select_related('page'):
                    if el.name and el.page and el.page.module_id is not None:
                        name_to_modules.setdefault(el.name, set()).add(el.page.module_id)
            except Exception as e:
                logger.warning(f"场景质量门构建元素映射失败，跳过模块校验：{e}")
                name_to_modules = {}
        ...
        for case in test_cases:
            ...
            if len(steps) < MIN_STEPS:
                reason = f'步骤数 {len(steps)} 步 < 场景用例要求的 {MIN_STEPS} 步'

            # 规则2：跨模块数下限——通过 step.target 反查元素所属模块
            # 仅当能反查到至少一个模块时才严格校验跨模块数；
            # 完全反查不到（说明 LLM 用了自由文本 target）时给予豁免，避免误杀
            if not reason and name_to_modules:
                involved: Set[int] = set()
                for step in steps:
                    target = (step.get('target') or '').strip()
                    if target and target in name_to_modules:
                        involved |= name_to_modules[target]
                if involved and len(involved) < MIN_MODULES:
                    reason = f'仅触达 {len(involved)} 个模块（要求 ≥ {MIN_MODULES} 个），疑似单模块用例'

            if reason:
                case['_quality_flag'] = {'ok': False, 'reason': reason}
                discarded.append({
                    'title': case.get('title', '(未命名用例)'),
                    'reason': reason
                })
            else:
                case['_quality_flag'] = {'ok': True, 'reason': ''}

            flagged.append(case)

        return flagged, discarded
    ...
    def _save_to_database_node(self, state: WebUITestCaseGeneratorState) -> WebUITestCaseGeneratorState:
        ...
        try:
            ...
            # 场景模式：先经过质量门，不达标的只打标不剔除，最终是否保留交由用户决定
            quality_warnings: List[str] = []
            if (state.get("test_case_type") or "module") == "scenario":
                related_ids = state.get("related_module_ids") or []
                flagged, discarded = self._scenario_quality_gate(test_cases, related_ids)
                if discarded:
                    logger.info(
                        f"场景质量门标记 {len(discarded)} 条不达标用例（保留在候选中由用户决定）：{discarded}"
                    )
                    for rec in discarded:
                        quality_warnings.append(
                            f'用例「{rec.get("title", "")}」已标记为不达标：{rec.get("reason", "")}'
                        )
                test_cases = flagged
            else:
                # 非场景模式：补一个默认 ok 的 _quality_flag，便于前端统一渲染
                for case in test_cases:
                    case.setdefault('_quality_flag', {'ok': True, 'reason': ''})
            # 写到 state，供 _run_with_langgraph 汇总到 warnings
            state["scenario_quality_warnings"] = quality_warnings
            ...
            # 前端显式传入的模块 ID（用户通过 @ 选中的业务模块），作为统一兜底
            preferred_module_id = state.get("module_id")
            if preferred_module_id is not None:
                try:
                    preferred_module_id = int(preferred_module_id)
                except (TypeError, ValueError):
                    preferred_module_id = None
                if preferred_module_id is not None and valid_module_ids and preferred_module_id not in valid_module_ids:
                    preferred_module_id = None

            # 本次生成的用例类型：'module' 或 'scenario'，影响 module 兜底与最终 test_case_type
            requested_type = state.get("test_case_type") or "module"
            auto_save = bool(state.get("auto_save"))

            created_test_cases: List[Dict[str, Any]] = []
            ...
            for i, test_case_data in enumerate(test_cases, 1):
                ...
                safe_module_id: Optional[int] = None

                # 1. 优先使用 LLM 返回的 module_id（如果合法）
                if raw_module_id not in [None, "", "null"]:
                    try:
                        candidate = int(raw_module_id)
                    except (TypeError, ValueError):
                        candidate = None
                    if candidate is not None and (not valid_module_ids or candidate in valid_module_ids):
                        safe_module_id = candidate

                # 2. 如果 LLM 没给出合法 module_id，且本次是 module 模式，回退为用户在前端通过 @ 选中的模块
                if safe_module_id is None and preferred_module_id is not None and requested_type == "module":
                    safe_module_id = preferred_module_id

                # 3. 按业务互斥规则强制写 test_case_type（保证 model=NULL ⇔ type=scenario）
                if requested_type == "scenario":
                    # 跨模块场景用例：强制清空 module
                    safe_module_id = None
                    safe_type = "scenario"
                else:
                    # 模块用例：极端兜底——若仍拿不到 module，则降级为 scenario 入库，
                    # 避免落出违反互斥规则的脏数据
                    safe_type = "module" if safe_module_id else "scenario"

                resolved_title = test_case_data.get('title', f'测试用例_{timestamp}_{i:03d}')
                resolved_description = test_case_data.get('description', 'AI生成的Web UI测试用例')
                resolved_url = test_case_data.get('url', '')
                resolved_priority = test_case_data.get('priority', 'medium')
                resolved_category = test_case_data.get('category', 'functional')
                resolved_preconditions = test_case_data.get('preconditions', [])
                resolved_steps = test_case_data.get('steps', [])
                resolved_expected = test_case_data.get('expected_result', '')
                quality_flag = test_case_data.get('_quality_flag') or {'ok': True, 'reason': ''}

                # 断言 DSL 过滤：保留合法的，丢弃非法（防止 LLM 写错）
                try:
                    from apps.web_testing.assertion_schema import (
                        validate_assertions as _validate_assertions,
                        dump_assertion as _dump_assertion,
                    )
                    raw_expectations = test_case_data.get('expectations') or []
                    cleaned_expectations = [
                        _dump_assertion(s) for s in _validate_assertions(raw_expectations)
                    ]
                    cleaned_steps = []
                    for st in resolved_steps:
                        if isinstance(st, dict):
                            inner_raw = st.get('assertions') or []
                            inner_clean = [_dump_assertion(s) for s in _validate_assertions(inner_raw)]
                            cleaned_steps.append({**st, 'assertions': inner_clean})
                        else:
                            cleaned_steps.append(st)
                except Exception as _e:  # noqa: BLE001
                    logger.warning(f"断言 DSL 校验失败，按空列表降级处理: {_e}")
                    cleaned_expectations = []
                    cleaned_steps = resolved_steps
                resolved_steps = cleaned_steps
                resolved_expectations = cleaned_expectations
            ...
            state["candidate_test_cases"] = candidate_test_cases
            state["test_cases_count"] = len(created_test_cases) if auto_save else 0
            if auto_save:
                logger.info(f"成功保存 {len(created_test_cases)} 个测试用例到数据库（auto_save=True）")
            else:
                logger.info(
                    f"生成 {len(candidate_test_cases)} 个候选测试用例（auto_save=False，待人工审核入库）"
                )

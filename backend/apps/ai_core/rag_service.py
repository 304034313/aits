...
class VectorDBConfig:
    ...
    def __init__(self):
        ...
        # 嵌入模型加载方式（双轨制）
        self.embedding_load_mode = 'remote'   # 'local' | 'remote'
        self.embedding_model_local_path = ''  # mode=local 时使用
        
        # Chroma配置（调试模式）
        self.chroma_persist_directory = getattr(
            settings, 'CHROMA_PERSIST_DIR', os.path.join(settings.BASE_DIR, 'chroma_db')
        )
...
class RAGManager:
    ...
    def __init__(self):
        ...
        self.embedding_model = None
        # 嵌入模型最近一次加载的元信息（mode_actual / fallback / device 等）
        self._embedding_load_meta: Dict[str, Any] = {}
    ...
    def _update_config_from_db(self, config: 'RAGConfiguration'):
        ...
        field_mapping = {
            ...
            # 嵌入模型加载模式（local/remote）+ 本地路径
            'embedding_load_mode': 'embedding_load_mode',
            'embedding_model_local_path': 'embedding_model_local_path',
        }
    ...
    def _initialize_embedding_model(self):
        """初始化嵌入模型（统一走 embedding_loader.get_embedding_model 双轨加载）。

        - 上层无需关心模型来源（local/remote）
        - 加载结果元信息保存到 ``self._embedding_load_meta``，供 test_connection
          / 前端降级通知使用
        - 加载失败抛 RuntimeError，由调用方决定是否兜底
        """
        if not self.config.embedding_model:
            error_msg = "嵌入模型配置缺失。请在前端RAG配置页面设置嵌入模型。"
            logger.error(error_msg)
        ...
        # 显式构造 dict 传给 loader，避免 loader 重复回查数据库
        loader_config = {
            'embedding_load_mode': getattr(self.config, 'embedding_load_mode', 'remote'),
            'embedding_model_local_path': getattr(self.config, 'embedding_model_local_path', ''),
            'embedding_model_remote_name': self.config.embedding_model,
        }
        try:
            from .embedding_loader import get_embedding_model
            result = get_embedding_model(loader_config)
        except Exception as e:
            logger.error(f"嵌入模型加载失败: {e}", exc_info=True)
            raise RuntimeError(f"嵌入模型初始化失败: {e}") from e

        self.embedding_model = result.embeddings
        self._embedding_load_meta = {
        ...
        try:
            ...
            logger.warning(f"探测嵌入向量维度失败，沿用配置 milvus_dim={self.config.milvus_dim}: {e}")
            actual_dimension = self.config.milvus_dim

        logger.info(
            "嵌入模型就绪: identifier=%s | mode_actual=%s (requested=%s) | device=%s | dim=%s | fallback=%s",
            result.model_identifier, result.mode_actual, result.mode_requested,
            result.device, actual_dimension, result.fallback,
        )
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                # 嵌入模型加载元信息：前端可据此显示"已降级"通知
                'embedding_load_meta': dict(self._embedding_load_meta or {}),
            }

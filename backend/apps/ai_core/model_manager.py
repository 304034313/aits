...
class ModelManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        active_config = LLMConfiguration.objects.filter(
            model_type=self.model_type, is_active=True, is_default=True
    ...
    def _initialize_model(self):
        ...
        if provider == 'ollama':
            ...
            # 走 Ollama 的 OpenAI 兼容接口，确保 base_url 以 /v1 结尾
            base_url = (self.config.get('base_url') or 'http://localhost:11434').rstrip('/')
            if not base_url.endswith('/v1'):
                base_url = f"{base_url}/v1"

            # 本地大模型生成长 JSON 非常耗时，这里显式将超时时间拉长到 600 秒（10 分钟）
            init_params.update({
                'model_provider': 'openai',
                'api_key': 'ollama',  # 绕过 OpenAI SDK 的 api_key 非空校验
                'base_url': base_url,
                'timeout': 600,
            })
    ...
    def _check_ollama_service(self, base_url: str) -> bool:
        """检查Ollama服务是否可用"""
        try:
            # 兼容用户在前端填写带有 /v1 的 OpenAI 兼容地址（如 http://localhost:11434/v1）
            # Ollama 自身的健康检查接口位于 /api/tags，因此需要去掉末尾的 /v1 以防 404/400
            normalized_base = (base_url or '').rstrip('/')
            if normalized_base.endswith('/v1'):
                normalized_base = normalized_base[:-3]  # 去掉末尾的 "/v1"
            response = requests.get(f"{normalized_base}/api/tags", timeout=5)
    ...
    def invoke(self, messages: List[Union[HumanMessage, SystemMessage]], **kwargs) -> str:
        ...
        try:
            logger.info(
                "🚀 [LLM调用开始] 模型名称: %s | 接口地址: %s | 提供商: %s",
                self.config.get('model'),
                self.config.get('base_url', '默认'),
    ...
    def _send_websocket_message(self, user_id: int, step_name: str, content: str, room_type: str) -> bool:
        ...
        try:
            ...
            success = websocket_message_service.send_streaming_output(
                ...
                timestamp=timestamp,
                room_type=room_type,
                model_name=self.config.get('model') if self.config else None,
                model_provider=self.config.get('provider') if self.config else None,
            )
            if not success:

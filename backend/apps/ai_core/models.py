...
class LLMConfiguration(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        如果当前配置被标记为默认（is_default=True），则自动将同模型类型下的
        其他配置的 is_default 置为 False，确保同一 model_type 仅有一个默认配置。
        """
        if self.is_default:
...
class RAGConfiguration(models.Model):
    ...
    # 嵌入模型配置（双轨：本地离线 / 远程下载）
    EMBEDDING_LOAD_MODE_CHOICES = (
        ('remote', '远程加载（联网下载/缓存）'),
        ('local', '本地离线加载'),
    )
    embedding_load_mode = models.CharField(
        max_length=10,
        choices=EMBEDDING_LOAD_MODE_CHOICES,
        default='remote',
    ...
    embedding_model_local_path = models.CharField(
        max_length=500,
        blank=True,
        default='',

...
class RAGConfigurationViewSet(APIView):
    ...
    def get(self, request):
        ...
        configs = RAGConfiguration.objects.all() if is_admin_user(request.user) else RAGConfiguration.objects.filter(created_by=request.user)
        configs = configs.order_by('-is_default', '-is_active', '-created_at')
    ...
    def post(self, request):
        ...
        existing_config = RAGConfiguration.objects.filter(created_by=request.user).first() if not is_admin_user(request.user) else None
        if existing_config:
            return response(
                kind="error",
...
class RAGConfigurationActionView(APIView):
    ...
    def post(self, request, config_id, action):
        ...
        if action == 'set_default':
            # 设置默认配置
            rag_qs = RAGConfiguration.objects.filter(is_default=True)
            if not is_admin_user(request.user):
                rag_qs = rag_qs.filter(created_by=request.user)
            rag_qs.update(is_default=False)
...
class RAGEmbeddingPathCheckView(APIView):
    """嵌入模型本地路径校验接口。

    给前端"检测路径"按钮使用：保存配置之前能即时反馈路径是否合法。
    """
    ...
    def post(self, request):
        ...
        if not path:
            return response(kind="validation_error", message="path 不能为空")
        try:
            from .embedding_loader import validate_local_path
            info = validate_local_path(path)
        except Exception as e:
            logger.error(f"校验本地嵌入模型路径失败: {e}", exc_info=True)
            return response(kind="error", message=f"校验失败: {e}")
        # 即使 valid=False 也按 success 返回，由前端依据 valid/message 决定 UI 表现
        return response(kind="success", data=info, message=info.get('message') or '路径检测完成')
...
class LLMConfigurationViewSet(APIView):
    ...
    def get(self, request):
        ...
        configs = LLMConfiguration.objects.all() if is_admin_user(request.user) else LLMConfiguration.objects.filter(created_by=request.user)
        configs = configs.order_by('-is_active', '-created_at')
...
class LLMConfigurationActionView(APIView):
    ...
    def post(self, request, config_id, action):
        ...
        if action == 'set_default':
            # 同类型下仅一条默认；默认配置自动启用以便 ModelManager 能选中
            llm_default_qs = LLMConfiguration.objects.filter(model_type=config.model_type).exclude(pk=config.pk)
            if not is_admin_user(request.user):
                llm_default_qs = llm_default_qs.filter(created_by=request.user)
            llm_default_qs.update(is_default=False)
            config.is_default = True
...
class LLMUsageStatisticsView(APIView):
    ...
    def get(self, request):
        ...
        user_logs = LLMUsageLog.objects.all() if is_admin_user(request.user) else LLMUsageLog.objects.filter(configuration__created_by=request.user)
...
class VisionTestConnectionView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                vt_kwargs = {'pk': config_id, 'model_type': 'vision'}
                if not is_admin_user(request.user):
                    vt_kwargs['created_by'] = request.user
                configuration = LLMConfiguration.objects.get(**vt_kwargs)
            except LLMConfiguration.DoesNotExist:
                return response(
                    kind="error",
                    message="视觉模型配置不存在"
...
class VisionSetDefaultView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            try:
                vs_kwargs = {'pk': pk, 'model_type': 'vision'}
                if not is_admin_user(request.user):
                    vs_kwargs['created_by'] = request.user
                configuration = LLMConfiguration.objects.get(**vs_kwargs)
            except LLMConfiguration.DoesNotExist:
                return response(
                    kind="error",
                    message="视觉模型配置不存在"
            ...
            vd_qs = LLMConfiguration.objects.filter(model_type='vision', is_default=True).exclude(pk=pk)
            if not is_admin_user(request.user):
                vd_qs = vd_qs.filter(created_by=request.user)
            vd_qs.update(is_default=False)
...
class VisionToggleActiveView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            try:
                vta_kwargs = {'pk': pk, 'model_type': 'vision'}
                if not is_admin_user(request.user):
                    vta_kwargs['created_by'] = request.user
                configuration = LLMConfiguration.objects.get(**vta_kwargs)
            except LLMConfiguration.DoesNotExist:
                return response(
                    kind="error",
                    message="视觉模型配置不存在"
...
class MCPConfigurationViewSet(APIView):
    ...
    def get(self, request):
        """获取MCP配置列表"""
        try:
            configurations = MCPConfiguration.objects.all() if is_admin_user(request.user) else MCPConfiguration.objects.filter(created_by=request.user)
...
class MCPConfigurationActionView(APIView):
    ...
    def post(self, request, config_id, action):
        ...
        if action == 'toggle_active':
            ...
            # 设置默认配置
            mcp_default_qs = MCPConfiguration.objects.filter(is_default=True)
            if not is_admin_user(request.user):
                mcp_default_qs = mcp_default_qs.filter(created_by=request.user)
            mcp_default_qs.update(is_default=False)
            config.is_default = True
            config.save()

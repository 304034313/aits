...
def update_task_progress(task_instance, progress: int, message: str):
    """更新任务进度"""
    task_id = getattr(getattr(task_instance, "request", None), "id", None)
            ...
            if result.successful():
                ...
                try:
                    ...
                    _payload = {
                        ...
                        "runId": "pre-fix-1",
                        "hypothesisId": "H4",
                        "location": "backend/apps/common/task/celery_task_utils.py:get_celery_task_status:failure",
                        "message": "AsyncResult indicates FAILURE",
                        "data": {
                            "task_id": task_id,
                            "task_state": task_state,
                            "error_info_has_task_id": isinstance(error_info, dict) and "task_id" in error_info,
                            "error_message": error_message,
                        },
                        "timestamp": __import__("time").time(),
                    }
                    with open("debug-213959.log", "a", encoding="utf-8") as _f:

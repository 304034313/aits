...
# ============ Redis 同步旁路 (绕开 eventlet+asyncio 嵌套阻塞) ============
_SYNC_REDIS_CLIENT_CACHE: Dict[str, Any] = {}


def _get_sync_redis_client_for_layer(layer):
    """为 RedisChannelLayer 返回一个进程级缓存的同步 redis-py 客户端。

    在 Celery eventlet 池下运行时，redis-py 的 socket 会被 eventlet 的 GreenSocket
    替换，与 hub 配合做协作式 I/O，调用本身仍是毫秒级的。
    """
    try:
        if not getattr(layer, "hosts", None):
            return None
        host = layer.hosts[0]
        if isinstance(host, dict):
            url = host.get("address") or host.get("url") or host.get("host")
        else:
            url = host
        if not url:
            return None
        cli = _SYNC_REDIS_CLIENT_CACHE.get(url)
        if cli is None:
            import redis as _redis  # 同步分支
            cli = _redis.Redis.from_url(url) if isinstance(url, str) else _redis.Redis(**host)
            _SYNC_REDIS_CLIENT_CACHE[url] = cli
        return cli
...
def _sync_redis_group_send(channel_layer, group: str, message: Dict[str, Any]) -> bool:
    """绕过 asyncio，用同步 redis-py 按 channels_redis 4.x 协议派发 group 消息。

    成功返回 True；不适用（非 RedisChannelLayer / 没有客户端 / 异常）返回 False，
    由调用方退回 async_to_sync 路径。
    """
    try:
    ...
    try:
        ...
        client = _get_sync_redis_client_for_layer(channel_layer)
        if client is None:
            return False

        group_key = channel_layer._group_key(group)  # bytes，形如 b"asgi:group:<group>"
        now = _time.time()
        # 与 channels_redis 一致：清理过期 group 成员
        client.zremrangebyscore(group_key, 0, int(now) - int(channel_layer.group_expiry))
        raw_channels = client.zrange(group_key, 0, -1) or []
        channels = [c.decode("utf8") if isinstance(c, (bytes, bytearray)) else c for c in raw_channels]
        if not channels:
            return True  # 没有订阅者直接成功

        prefix = channel_layer.prefix
        expiry = int(channel_layer.expiry)
        pipe = client.pipeline(transaction=False)
        for channel in channels:
            non_local = channel_layer.non_local_name(channel)
            channel_key = prefix + non_local  # 与 send() 一致：prefix + non_local（无分隔符）
            per_msg = dict(message)
            # channels_redis 在 group_send 内部强制注入；保持兼容
            per_msg["__asgi_channel__"] = [channel]
            value = channel_layer.serialize(per_msg)  # 复用层自身的序列化（含可选加密 + 12 字节随机前缀）
            pipe.zremrangebyscore(channel_key, 0, int(now) - expiry)
            pipe.zadd(channel_key, {value: now})
            pipe.expire(channel_key, expiry)
        pipe.execute()
        return True
    except Exception as e:
        logger.warning(f"sync_redis_group_send 失败，将回退 async_to_sync: {e}")
        return False
...
class StreamingConsumerMixin:
    ...
    async def tool_call(self, event):
        """发送 AI 工具调用事件（AI 操作回放）"""
        try:
            await self.send(text_data=json.dumps({
                'type': WebSocketConfig.MESSAGE_TYPES['TOOL_CALL'],
                'tool_name': event.get('tool_name', ''),
                'display_text': event.get('display_text', ''),
                'icon': event.get('icon', 'tool'),
            ...
            logger.error(f"发送工具调用事件时发生错误: {e}")
...
class WebSocketMessageService:
    ...
    def _send_async_message(self, room_group_name: str, message_data: Dict[str, Any]):
        """从同步上下文派发到 Channels 异步层。

        在 Celery eventlet 池下，``RedisChannelLayer.group_send`` 走 asyncio 会
        被 eventlet hub 反复让出/等待，实测每次 ~4.4s。这里检测到是
        RedisChannelLayer 时，走同步 redis-py 旁路按相同协议直接 ZADD，绕开
        asyncio↔eventlet 嵌套；其它 channel layer 退回 ``async_to_sync``。
        """
        try:
            try:
            ...
            if _sync_redis_group_send(self.channel_layer, room_group_name, message_data):
                return
            async_to_sync(self.channel_layer.group_send)(room_group_name, message_data)
        except Exception as e:
    ...
    def send_streaming_output(
        ...
        if model_provider:
            message_data['model_provider'] = model_provider
    ...
    def send_tool_call(
        ...
        """发送 AI 工具调用事件给前端 (AI 操作回放面板)。

        :param tool_name: 真实工具名，例如 'browser_navigate'
        :param display_text: 业务化人类可读文案，例如 "正在打开页面: http://..."
        :param icon: 前端 icon 提示 key（globe / mouse / keyboard / search / camera / list / tool ...）
        :param raw_input: 原始入参（已在调用方截断）
        :param raw_output: 原始 observation（已在调用方截断）
        """
        message_data = {
            'type': WebSocketConfig.MESSAGE_TYPES['TOOL_CALL'],
            'tool_name': tool_name,
        ...
        room_group_name = f"{room_type}_streaming_{user_id}"
        return self._send_message(room_group_name, message_data, f"工具调用事件: 用户{user_id}, 工具{tool_name}")

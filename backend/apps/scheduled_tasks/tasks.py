...
def _execute_app_test_suite(task: ScheduledTask, execution_log: TaskExecutionLog) -> Dict[str, Any]:
    """执行 App 定时任务：触发 Sonic 上已有测试套件（suite_ids 为 Sonic 套件 id）。"""
    try:
        from app_testing.tasks import execute_app_scheduled_suites_task

        raw_ids = task.suite_ids or []
        if not raw_ids:
            return build_error_result('未选择 App（Sonic）测试套件')

        sonic_suite_ids: list = []
        for x in raw_ids:
            try:
                sonic_suite_ids.append(int(x))
            except (TypeError, ValueError):
                return build_error_result(f'无效的 Sonic 测试套件 id: {x}')

        proj = task.project
        if not getattr(proj, 'sonic_project_id', None):
            return build_error_result('项目未配置 sonic_project_id，无法执行 App 定时任务')

        total_cases = len(sonic_suite_ids)
        celery_task = execute_app_scheduled_suites_task.delay(
            task.id,
            execution_log.id,
            sonic_suite_ids,
        )
        logger.info(
            'App 定时任务已投递 Sonic 执行: task_id=%s execution_log_id=%s celery_id=%s suites=%s',
            task.id,
            execution_log.id,
            celery_task.id,
            sonic_suite_ids,
        ...
        return {
            'success': True,
            'message': 'App（Sonic）测试套件执行已启动',
            'task_ids': [celery_task.id],
            'execution_ids': [],
            'total_cases': total_cases,
            'passed_cases': 0,
            'failed_cases': 0,

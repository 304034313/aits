...
class ScheduledTaskSerializer(serializers.ModelSerializer):
    ...
    def get_environment_name(self, obj):
        if getattr(obj, 'suite_type', None) == 'app':
            return ''
...
class ScheduledTaskCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ScheduledTask
        fields = [
            'name', 'description', 'suite_type', 'suite_ids', 'app_run_config', 'cron_expression',
            'environment', 'status', 'notice_targets', 'trigger_condition',
        ]

    def validate(self, attrs):
        suite_type = attrs.get('suite_type')
        if suite_type is None and getattr(self, 'instance', None):
            suite_type = self.instance.suite_type

        if suite_type == 'app':
            project_id = self.context.get('project_id')
            if project_id:
                proj = Project.objects.filter(pk=project_id).first()
                if not proj or not proj.sonic_project_id:
                    raise serializers.ValidationError(
                        {'suite_type': 'App 定时任务需要先在项目中配置 Sonic 项目 ID（sonic_project_id）'}
                    )
            attrs['environment'] = None
        elif suite_type in ('web', 'api'):
            env = attrs.get('environment')
            if env is None and getattr(self, 'instance', None):
                env = self.instance.environment
            if env is None:
                raise serializers.ValidationError(
                    {'environment': 'Web/API 定时任务必须选择执行环境'}
                )
        return attrs
    ...
    def validate_suite_ids(self, value):
        ...
        if suite_type == 'web':
            ...
            if not apps.is_installed('web_testing'):
                raise serializers.ValidationError('Web 测试模块不可用')
            ...
            from django.apps import apps
            if not apps.is_installed('app_testing'):
                raise serializers.ValidationError('App 测试模块不可用')
            from app_testing.services import SonicDeviceService, SonicProxyError
            ...
            for x in value:
                try:
                    ...
                    raise serializers.ValidationError(f'无效的 Sonic 测试套件 id: {x}') from exc
                if i <= 0:
                    raise serializers.ValidationError(f'无效的 Sonic 测试套件 id: {x}')
                normalized.append(i)
            project_id = self.context.get('project_id')
            if project_id:
                proj = Project.objects.filter(pk=project_id).first()
                if proj and proj.sonic_project_id:
                    try:
                        suites = SonicDeviceService().list_all_test_suites(int(proj.sonic_project_id))
                        valid_ids = {s['id'] for s in suites}
                        missing = [i for i in normalized if i not in valid_ids]
                        if missing:
                            raise serializers.ValidationError(
                                f'Sonic 项目中不存在以下测试套件 id: {missing}'
                            )
                    except SonicProxyError as exc:
                        raise serializers.ValidationError(f'无法从 Sonic 拉取套件列表: {exc.message}') from exc
            return normalized

        return value

    def validate_app_run_config(self, value):
        if value is None:
            return {}
        if not isinstance(value, dict):
            raise serializers.ValidationError('app_run_config 必须为 JSON 对象')
        return value

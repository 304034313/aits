...
class ScheduledTask(models.Model):
    ...
    # App 定时任务扩展：设备等信息（执行由 Sonic 完成，此处仅存配置）
    app_run_config = models.JSONField(
        default=dict,
        blank=True,
    ...
    # 环境配置（Web/API 必选；App 由 Sonic 执行，可为空）
        ...
        try:
            ...
            if self.suite_ids and len(self.suite_ids) > 0:
                ...
                if self.suite_type == 'web':
                    ...
                    suite_names = [suite.name for suite in suites]
                elif self.suite_type == 'app':
                    spid = getattr(self.project, 'sonic_project_id', None)
                    if spid:
                        try:
                            from app_testing.services import SonicDeviceService

                            svc = SonicDeviceService()
                            rows = svc.list_all_test_suites(int(spid))
                            id_to_name = {int(r['id']): r['name'] for r in rows}
                            for sid in self.suite_ids:
                                try:
                                    i = int(sid)
                                    suite_names.append(id_to_name.get(i, f'Sonic套件#{i}'))
                                except (TypeError, ValueError):
                                    suite_names.append(str(sid))
                        except Exception:
                            suite_names = []

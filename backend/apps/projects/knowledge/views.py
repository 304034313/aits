...
class KnowledgeBaseFileListView(AdminBypassMixin, generics.ListCreateAPIView):
    """知识库文件列表和上传视图"""
...
class KnowledgeBaseFileDetailView(AdminBypassMixin, generics.RetrieveUpdateDestroyAPIView):
    """知识库文件详情视图"""
    serializer_class = KnowledgeBaseFileSerializer
    permission_classes = [permissions.IsAuthenticated]
...
def reprocess_file(request, project_id, file_id):
    ...
    try:
        ...
        if not is_admin_user(request.user) and not file_obj.project.members.filter(user=request.user, can_edit=True).exists():
            return response(
                kind="permission_denied",
                message="权限不足",
                status_code=status.HTTP_403_FORBIDDEN
...
def start_file_processing(request, project_id, file_id):
    ...
    try:
        ...
        if not is_admin_user(request.user) and not project.members.filter(user=request.user, can_edit=True).exists():
            return response(
                kind="permission_denied",
                message="权限不足",
                status_code=status.HTTP_403_FORBIDDEN

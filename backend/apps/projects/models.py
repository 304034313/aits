...
class Project(models.Model):
    ...
    # Sonic 云真机项目 ID（App 测试前端直连 Sonic API 时使用，与 AITS 项目 id 不同）
    sonic_project_id = models.PositiveIntegerField(
        _('sonic project id'),
        null=True,
        blank=True,
    )
    ...
    # 性能测试项目可关联多个接口自动化项目（对称=False：仅「本端」维护关联）
    linked_api_projects = models.ManyToManyField(
        'self',
        symmetrical=False,
        blank=True,
        related_name='performance_projects',
        verbose_name=_('关联的接口项目'),
    )
...
class ProjectThirdPartyConfig(models.Model):
    """项目第三方系统集成配置（每项目唯一）。"""

    project = models.OneToOneField(
        Project,
        on_delete=models.CASCADE,
        related_name='third_party_config',
        verbose_name='所属项目',
    )
    grafana_dashboard_url = models.URLField(
        max_length=1000,
        null=True,
        blank=True,
        verbose_name='性能大盘地址',
    )
    ai_diagnosis_url = models.URLField(
        max_length=1000,
        null=True,
        blank=True,
        verbose_name='AI诊断大盘地址',
    )
    grafana_admin_url = models.URLField(
        max_length=1000,
        null=True,
        blank=True,
        verbose_name='Grafana后台地址',
    )
    prometheus_url = models.URLField(
        max_length=1000,
        null=True,
        blank=True,
        verbose_name='Prometheus地址',
    )
    skywalking_url = models.URLField(
        max_length=1000,
        null=True,
        blank=True,
        verbose_name='SkyWalking地址',
    )
    updated_at = models.DateTimeField(_('updated at'), auto_now=True)

    class Meta:
        db_table = 'project_third_party_configs'
        verbose_name = _('project third-party config')
        verbose_name_plural = _('project third-party configs')
        ordering = ['-updated_at', '-id']

    def __str__(self):
        return f"{self.project.name} third-party config"
...
class WorkspaceMenuConfig(models.Model):
    """工作区侧边栏菜单配置（管理员统一设定，全员生效）"""
    ...
    module = models.CharField(_('module'), max_length=32, choices=MODULE_CHOICES, unique=True)
    menu_items = models.JSONField(_('menu items'), default=list)
    ...
    class Meta:
        ...
        verbose_name = _('workspace menu config')
        verbose_name_plural = _('workspace menu configs')

    def __str__(self):
        return f"MenuConfig({self.module})"
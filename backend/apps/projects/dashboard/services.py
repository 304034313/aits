...
def _web_testing_available():
    return apps.is_installed('web_testing')
...
def _empty_summary():
    return {
        ...
        'ai_cases_count': 0,
        'total_cases': 0,
        'ai_metric_applicable': False,
    }


def _ai_contribution_from_cases(project_id, case_model):
    """AI 创建用例占比：created_source='ai' / 项目总用例数。"""
    total_cases = case_model.objects.filter(project_id=project_id).count()
    ai_cases = case_model.objects.filter(project_id=project_id, created_source='ai').count()
    rate = round((ai_cases / total_cases * 100), 2) if total_cases > 0 else 0.0
    return {
        'ai_contribution_rate': rate,
        'ai_cases_count': ai_cases,
...
def _api_summary(project_id):
    ...
    ai_stats = _ai_contribution_from_cases(project_id, APITestCase)
...
def _web_summary(project_id):
    ...
    today_pass_rate = round((passed_steps / total_steps * 100), 2) if total_steps > 0 else 0.0
    ai_stats = _ai_contribution_from_cases(project_id, WebUITestCase)

      ...
      >
        ...
        >
          ...
          >
            ...
            <div class="metric-info">
              <span class="metric-label">
                {{ getMetricLabel(item.i) }}
                <el-tooltip
                  v-if="item.i === 'metric-ai-rate'"
                  placement="top"
                  :show-after="200"
                >
                  <template #content>
                    <div class="metric-ai-tooltip">
                      统计当前项目用例库中，由 AI 功能创建并入库的用例占比。<br />
                      API：含「端点 AI 生成」「AI 场景智能体」等入口。<br />
                      Web：含「测试用例生成智能体」审核入库的用例。<br />
                      不含：仅 AI 生成脚本（未新建用例）、AI 脚本实验室探索、手工新建、Postman 导入。<br />
                      不是代码覆盖率，也不是近 7 日执行过的用例比例。
                    </div>
                  </template>
              ...
              <span v-if="item.i === 'metric-ai-rate' && getMetricSubline(item.i)" class="metric-subline">
                {{ getMetricSubline(item.i) }}
              </span>
            </div>
          </div>
          ...
          <div
            ...
            :class="[item.i, { 'edit-border': editMode, 'portal-card--disabled': isPortalDisabled(item.i) }]"
            ...
            <div class="portal-card-inner">
              ...
              <div class="portal-content">
                <h3 class="portal-title">
                  {{ getPortalTitle(item.i) }}
                  <span v-if="isPortalDisabled(item.i)" class="portal-badge">完整版</span>
                </h3>
...
function getMetricValue(i) {
  ...
  if (i === 'metric-ai-rate') {
    if (s.ai_metric_applicable === false) return '—'
    return `${s.ai_contribution_rate ?? 0}%`
  }
  if (i === 'metric-total-cases') return String(s.total_cases)
  return '0'
}

function getMetricSubline(i) {
  if (i !== 'metric-ai-rate') return ''
  const s = summary.value
  if (s.ai_metric_applicable === false) {
    return '当前项目类型暂未接入'
  }
  const ai = s.ai_cases_count ?? 0
  const total = s.total_cases ?? 0
  return `AI 创建 ${ai} / 共 ${total}`
}
  ...
  try {
    ...
    if (data) {
      summary.value = {
        ...
        ai_cases_count: data.ai_cases_count ?? 0,
        total_cases: data.total_cases ?? 0,
        ai_metric_applicable: data.ai_metric_applicable !== false,
      }
    }
  } catch (e) {
...
async function handlePortalClick(i) {
  if (isPortalDisabled(i)) {
...
.portal-card.portal-card--disabled:hover {
  transform: none;
  box-shadow: var(--cockpit-card-shadow);
}

.portal-card.portal-card--disabled:hover .portal-glow-bar::after {
  display: none;
}

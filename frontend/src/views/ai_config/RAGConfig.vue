  ...
  <div class="rag-config-page">
    ...
    <el-card class="config-card" v-loading="loading" v-if="currentConfig">
      ...
      <div class="config-content">
        <div class="config-section">
          ...
          <div class="config-item">
            <span class="config-label">嵌入模型（远程名）</span>
            <span class="config-value">{{ currentConfig.embedding_model }}</span>
          </div>
          <div class="config-item">
            <span class="config-label">加载模式</span>
            <el-tag
              :type="currentConfig.embedding_load_mode === 'local' ? 'success' : 'info'"
              class="config-value-tag">
              {{ currentConfig.embedding_load_mode_display || (currentConfig.embedding_load_mode === 'local' ? '本地离线加载' : '远程加载（联网下载/缓存）') }}
            </el-tag>
          </div>
          <div class="config-item" v-if="currentConfig.embedding_load_mode === 'local'">
            <span class="config-label">本地模型路径</span>
            <span class="config-value" style="word-break: break-all">{{ currentConfig.embedding_model_local_path || '（未设置）' }}</span>
          </div>
          <div class="config-item">
    ...
    <el-dialog v-model="showCreateRAGDialog" :title="editingRAGConfig ? '编辑RAG配置' : '添加RAG配置'" width="700px"
      ...
      <el-form ref="ragConfigFormRef" :model="ragConfigForm" :rules="ragConfigRules" label-width="140px">
        ...
        <!-- 嵌入模型加载模式（双轨制：本地离线 / 远程下载） -->
        <el-form-item label="加载模式" prop="embedding_load_mode">
          <el-radio-group v-model="ragConfigForm.embedding_load_mode">
            <el-radio value="remote">远程加载（联网下载/缓存）</el-radio>
            <el-radio value="local">本地离线加载</el-radio>
          </el-radio-group>
          <div class="form-item-hint">
            本地模式断网可用；加载失败会自动降级到下方"远程模型名"。
          </div>
        </el-form-item>

        <el-form-item
          :label="ragConfigForm.embedding_load_mode === 'local' ? '降级远程模型' : '嵌入模型'"
          ...
          <div class="form-item-hint" v-if="ragConfigForm.embedding_load_mode === 'local'">
            本地路径无效或加载失败时，将自动降级使用此远程模型。
          </div>
        </el-form-item>

        <!-- 仅本地模式显示：本地路径 + 检测按钮 -->
        <el-form-item
          v-if="ragConfigForm.embedding_load_mode === 'local'"
          label="本地模型路径" prop="embedding_model_local_path">
          <el-input
            v-model="ragConfigForm.embedding_model_local_path"
            placeholder="例如 D:/dev/models/bge-large-zh-v1.5"
            clearable>
            <template #append>
              <el-button :loading="checkingPath" @click="onCheckEmbeddingLocalPath">
                检测路径
              </el-button>
            </template>
          </el-input>
          <div
            v-if="pathCheckResult"
            class="path-check-result"
            :class="pathCheckResult.valid ? 'is-valid' : 'is-invalid'">
            <el-icon v-if="pathCheckResult.valid"><CircleCheck /></el-icon>
            <el-icon v-else><WarningFilled /></el-icon>
            <span>{{ pathCheckResult.message }}</span>
          </div>
        </el-form-item>
...
import { DataAnalysis, Plus, ArrowDown, CircleCheck, WarningFilled } from '@element-plus/icons-vue'
...
const pathCheckResult = ref(null) // { valid, exists, has_config_json, has_model_file, message, ... }
...
const ragConfigRules = {
  ...
  embedding_load_mode: [
    { required: true, message: '请选择加载模式', trigger: 'change' }
  ],
  embedding_model_local_path: [
    {
      validator: (rule, value, callback) => {
        if (ragConfigForm.embedding_load_mode === 'local' && !value) {
          callback(new Error('本地模式下必须填写模型文件夹绝对路径'))
        } else {
          callback()
        }
      },
...
const editRAGConfiguration = (config) => {
  ...
  resetForm(ragConfigForm, {
    ...
    embedding_load_mode: config.embedding_load_mode || 'remote',
...
// "检测路径"按钮：调后端校验本地模型目录
const onCheckEmbeddingLocalPath = async () => {
  const path = (ragConfigForm.embedding_model_local_path || '').trim()
  if (!path) {
    ElMessage.warning('请先填写本地模型路径')
    return
  }
  try {
    checkingPath.value = true
    const resp = await checkEmbeddingLocalPath(path)
    const data = resp?.data?.data || resp?.data || {}
    pathCheckResult.value = data
    if (data.valid) {
      ElMessage.success('路径有效，可以保存配置')
    } else {
      ElMessage.warning(data.message || '路径无效')
    }
  } catch (error) {
    console.error('检测嵌入模型路径失败:', error)
    pathCheckResult.value = {
      valid: false,
      message: error.response?.data?.message || '检测失败，请稍后重试'
    }
    ElMessage.error('检测嵌入模型路径失败')
  } finally {
    checkingPath.value = false
  }
}

// 监听加载模式切换：清空校验结果，避免误导
watch(() => ragConfigForm.embedding_load_mode, () => {
  pathCheckResult.value = null
})
...
const testRAGConnectionHandler = async (config) => {
  try {
    ...
    if (data && data.success) {
      ...
      // 嵌入模型若发生降级（mode=local 但实际走了 remote），弹非阻塞通知
      const meta = data.embedding_load_meta
      if (meta && meta.fallback) {
        ElNotification.warning({
          title: '嵌入模型已降级',
          message: `请求模式：${meta.mode_requested}，实际加载：${meta.mode_actual}。原因：${meta.fallback_reason || '本地路径不可用'}`,
          duration: 0,
...
.path-check-result.is-valid {
  color: #67c23a;
}
.path-check-result.is-invalid {
  color: #e6a23c;
}
</style>

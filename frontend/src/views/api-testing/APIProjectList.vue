            ...
            <el-card shadow="hover" class="project-card">
              ...
              <div class="card-footer">
                ...
                <div class="card-actions">
                  <el-button size="small" @click="openEditDialog(project)">编辑</el-button>
                  <el-button type="danger" size="small" plain @click="confirmDelete(project)">删除</el-button>
      ...
      >
        <el-form ref="editFormRef" :model="editForm" :rules="editRules" label-width="80px">
          <el-form-item label="项目名称" prop="name">
            <el-input v-model="editForm.name" placeholder="请输入项目名称" maxlength="50" show-word-limit />
          </el-form-item>
...
const submitEdit = async () => {
  ...
  await editFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      resetEditForm()
      await loadProjects()
      if (projectStore.currentProject?.id === savedId) {
        const refreshed = projectList.value.find(p => p.id === savedId)
        if (refreshed) await projectStore.setCurrentProject(refreshed)
      }
    } catch (error) {
      const d = error.response?.data
      ElMessage.error(d?.message || (typeof d?.detail === 'string' ? d.detail : null) || '保存失败')
    } finally {
...
const confirmDelete = async (project) => {
  try {
    await ElMessageBox.confirm(
      `确定要删除项目「${project.name}」吗？删除后项目下所有数据将不可恢复。`,
      '删除确认',
    ...
    if (projectStore.currentProject?.id === project.id) {
      await projectStore.clearCurrentProject()
    }
    await loadProjects()
  } catch (e) {
    if (e !== 'cancel') {
      const d = e.response?.data
      ElMessage.error(d?.message || (typeof d?.detail === 'string' ? d.detail : null) || '删除失败')
    }
  }
}

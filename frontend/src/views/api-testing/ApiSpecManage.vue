  ...
  <el-card v-if="selectedProject" class="files-card">
    ...
    <el-dialog v-model="showUploadDialog" title="上传API规范文档" width="500px">
      ...
      <el-form ref="uploadFormRef" :model="uploadForm" :rules="uploadRules" label-width="100px"
        ...
        <!-- Postman 专属导入选项 -->
        <template v-if="uploadForm.spec_type === 'postman'">
          <el-form-item label="导入模式" prop="import_mode">
            <el-radio-group v-model="uploadForm.import_mode">
              <el-radio value="spec_only">仅生成规范</el-radio>
              <el-radio value="endpoint_cases">生成端点用例</el-radio>
              <el-radio value="scenario_case">生成场景用例</el-radio>
              <el-radio value="all">全部生成</el-radio>
            </el-radio-group>
            <div class="postman-mode-hint">
              <el-icon><InfoFilled /></el-icon>
              <span>{{ postmanModeHint }}</span>
            </div>
          </el-form-item>
          <el-form-item label="AI 翻译脚本">
            <el-switch v-model="uploadForm.enable_ai_translate" />
            <span class="postman-ai-hint">
              开启后，pm.test() 与 pre-request 的 JS 脚本会被 AI 翻译为 HttpRunner 原生 validate / extract / variables。
              如未配置 LLM，将自动降级为规则解析。
            </span>
          </el-form-item>
        </template>
...
const uploadForm = reactive({
  ...
  spec_type: 'swagger',
  spec_file: null,
  // Postman 专属字段（其他类型时由后端忽略）
  import_mode: 'spec_only',
  enable_ai_translate: true
})
...
// 不同 spec_type 接受的文件后缀
const uploadAcceptTypes = computed(() => {
  return uploadForm.spec_type === 'postman' ? '.json' : '.json,.yaml,.yml'
})

const uploadHintText = computed(() => {
  if (uploadForm.spec_type === 'postman') {
    return '请上传 Postman Collection v2.1 / v2.0 导出的 .json 文件，大小不超过 10MB'
  }
  return '支持 JSON、YAML 格式文件，文件大小不超过 10MB'
})

const postmanModeHint = computed(() => {
  const map = {
    spec_only: '仅生成 API 规范与端点，不创建任何测试用例。',
    endpoint_cases: '为每个 Postman 请求生成 1 个独立的端点测试用例（推荐）。',
    scenario_case: '将整个 Collection 按顺序合并为 1 个场景测试用例。',
    all: '同时生成端点用例与场景用例（耗时最长）。'
  }
  return map[uploadForm.import_mode] || ''
})
...
const handleFileChange = (file) => {
  // 根据 spec_type 决定允许的扩展名
  const isPostman = uploadForm.spec_type === 'postman'
  const allowedTypes = isPostman ? ['.json'] : ['.json', '.yaml', '.yml']
  const fileExtension = file.name.substring(file.name.lastIndexOf('.')).toLowerCase()

  if (!allowedTypes.includes(fileExtension)) {
    const tip = isPostman
      ? 'Postman Collection 必须是 .json 文件'
      : '不支持的文件格式，请上传 JSON 或 YAML 文件'
    ElMessage.error(tip)
...
const handleUploadParseSpec = async () => {
  try {
    ...
    if (uploadForm.spec_name) {
      formData.append('spec_name', uploadForm.spec_name)
    }
    if (uploadForm.description) {
      formData.append('description', uploadForm.description)
    }
    // Postman 专属字段（其他类型时后端会自动忽略）
    if (uploadForm.spec_type === 'postman') {
      formData.append('import_mode', uploadForm.import_mode)
      formData.append('enable_ai_translate', uploadForm.enable_ai_translate ? 'true' : 'false')
    }

    const resp = await uploadParseAPISpecification(projectStore.currentProjectId, formData)
    const taskId = resp?.data?.task_id
    const specId = resp?.data?.id

    if (taskId && specId) {
      ElMessage.success('Postman 导入任务已启动，将在后台处理...')
      // 复用现有的"AI 生成中"状态机来展示卡片进度
      aiGeneratingStates.value.set(specId, true)
      activeTaskIds.value.set(specId, taskId)
      pollTaskStatus(specId, taskId)
    } else {
    ...
    uploadForm.spec_file = null
    uploadForm.import_mode = 'spec_only'
    uploadForm.enable_ai_translate = true
    fileList.value = []

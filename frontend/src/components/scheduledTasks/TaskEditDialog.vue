  ...
  >
    ...
    >
      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <el-alert
            v-if="form.suite_type === 'app'"
            type="info"
            :closable="false"
            show-icon
            class="app-suite-hint"
            title="运行测试套件前要求测试套件已绑定在线设备，请在「测试套件」管理中为套件绑定设备后再执行定时任务。"
          />
          <el-form-item label="测试套件" prop="suite_ids">
            <el-select 
              v-model="form.suite_ids" 
              :placeholder="form.suite_type === 'app' ? '请选择 Sonic 测试套件' : '请选择测试套件'" 
              ...
              >
                <div class="suite-option">
                  ...
                  <div class="suite-info">
                    ...
                    <span v-if="form.suite_type === 'app'" class="suite-id">ID: {{ suite.id }}</span>
                  </div>
                </div>
              </el-option>
            </el-select>
      ...
      <!-- 执行环境（App 由 Sonic 执行，不需要 AITS 环境） -->
      <el-form-item v-if="form.suite_type && form.suite_type !== 'app'" label="执行环境" prop="environment">
        <el-select 
...
const formRules = computed(() => {
  const base = {
    ...
    suite_type: [{ required: true, message: '请选择测试类型', trigger: 'change' }],
    suite_ids: [
...
const loadEnvironments = async () => {
  if (!selectedProject.value || form.suite_type === 'app') {
    environments.value = []
    return
  }
...
const handleSubmit = async () => {
  try {
    ...
    const { project: _p, ...rest } = { ...form, app_run_config: {} }
    const data = rest

...
// 批量创建 WebUI 测试用例（用于「AI 候选用例人工审核后批量入库」流程）
// payload 形如 { test_cases: [ { title, description, url, priority, category, test_case_type, preconditions, steps, expected_result, module_id }, ... ] }
// 后端响应 data: { created_count, failed_count, created_ids, failed: [{index, errors}], created_test_cases }
export const batchCreateWebUITestCases = async (projectId, testCases) => {
  const response = await api.post(`/projects/${projectId}/web-testing/test-cases/batch-create/`, {
...
export const listLabExplorations = async (projectId) => {
  const response = await api.get(`/projects/${projectId}/web-testing/lab-explorations/`)
  return response.data
}

export const getLabExploration = async (projectId, id) => {
  const response = await api.get(`/projects/${projectId}/web-testing/lab-explorations/${id}/`)
  return response.data
}

export const deleteLabExploration = async (projectId, id) => {
  const response = await api.delete(`/projects/${projectId}/web-testing/lab-explorations/${id}/`)
  return response.data
}

export const runLabDraftScript = async (projectId, data) => {
  const response = await api.post(`/projects/${projectId}/web-testing/lab-explorations/run/`, data)
  return response.data
}
...
// 生成 Playwright 代码（单条）
export const generateTestCaseCode = async (projectId, testCaseId, framework = 'playwright', forceRegenerate = false) => {
  const response = await api.post(
    `/projects/${projectId}/web-testing/test-cases/${testCaseId}/generate-code/`,
    { framework, force_regenerate: !!forceRegenerate }
  )
  return response.data
}
...
// ============ Web Auth Profile（登录态档案） ============

export const getWebAuthProfiles = async (projectId, params = {}) => {
  const response = await api.get(`/projects/${projectId}/web-testing/auth-profiles/`, { params })
  return response.data
}

export const createWebAuthProfile = async (projectId, data) => {
  const response = await api.post(`/projects/${projectId}/web-testing/auth-profiles/`, data)
  return response.data
}

export const updateWebAuthProfile = async (projectId, profileId, data) => {
  const response = await api.put(`/projects/${projectId}/web-testing/auth-profiles/${profileId}/`, data)
  return response.data
}

export const deleteWebAuthProfile = async (projectId, profileId) => {
  const response = await api.delete(`/projects/${projectId}/web-testing/auth-profiles/${profileId}/`)
  return response.data
}

export const refreshWebAuthProfile = async (projectId, profileId, data = {}) => {
  const response = await api.post(
    `/projects/${projectId}/web-testing/auth-profiles/${profileId}/refresh/`,
    data
  )
  return response.data
}

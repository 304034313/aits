...
/**
 * 将指定 LLM 配置设为当前用户下同模型类型的默认（会启用该配置并取消其他默认）
 */
export function setLLMConfigurationDefault(id) {
  return api.post(`/ai-core/llm-configs/${id}/set_default/`)
}
...
/**
 * 校验嵌入模型本地路径（不入库，仅检测）
 * @param {string} path 待校验的本地模型文件夹绝对路径
 */
export function checkEmbeddingLocalPath(path) {
  return api.post('/ai-core/rag-configs/check_embedding_path/', { path })
}
